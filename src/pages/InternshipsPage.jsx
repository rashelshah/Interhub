...
const InternshipsPage = () => {
  ...
  useEffect(() => {
    let results = [...internships].map(internship => ({
      ...
      salaryValue: parseInt(internship.salary.replace('$', '')) * 83
    }));
    ...
    if (filters.search) {
      ...
      results = results.filter(
        internship => 
          ...
          (internship.tags && internship.tags.some(tag => tag.toLowerCase().includes(searchLower)))
      );
    }
        ...
        <form onSubmit={handleSearch} className="max-w-2xl mb-8">
          <div className="relative">
            <input
              ...
              className="w-full py-3 pl-4 pr-12 rounded-lg shadow-sm border-gray-300 focus:border-teal-600 focus:ring focus:ring-teal-600/20 focus:ring-opacity-50"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-teal-600 text-white p-2 rounded-md hover:bg-emerald-700 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1">
            ...
            <div className="mb-6">
              ...
              {/* Applied Filters */}
              {(filters.locations.length > 0 || filters.categories.length > 0 || filters.tags.length > 0 || filters.remoteOnly || filters.salary > 0) && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredInternships.length > 0 ? (
                ...
                <div className="col-span-full text-center py-12">
                  ...
                  <p className="mt-1 text-gray-600">Try adjusting your filters or search query.</p>
                  <button
                    onClick={() => setFilters({
                      search: '',
                      salary: 0,
                    ...
                    className="mt-4 text-teal-600 hover:text-emerald-700"
                  >
                    Clear all filters
                  </button>
                </div>
